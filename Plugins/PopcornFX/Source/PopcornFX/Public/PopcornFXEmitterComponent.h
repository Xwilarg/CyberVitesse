...
FWD_PK_API_BEGIN
class CActionInstance;
typedef TRefPtr<CActionInstance>	PActionInstance_;
template <typename _Type> class TTypedParameterList;
class CActionParameterDictionnary;
typedef	TTypedParameterList<CActionParameterDictionnary>	CActionParameters;
typedef TRefPtr<CActionParameters>	PActionParameters_;
FWD_PK_API_END
...
public:
	...
	/** If false, the emitter will not update the emission position neither attirubtues */
	...
	*/
	...
	UFUNCTION(BlueprintCallable, Category="PopcornFX|Emitter", meta=(Keywords="popcornfx particle emitter effect system", UnsafeDuringActorConstruction="true"))
	bool							ToggleEmitter(bool startEmitter);
	...
	*/
	...
	void							Scene_DuringUpdate(CParticleScene *scene, float deltaTime, enum ELevelTick tickType);
...
#endif
	void							_OnDeathNotifier(const PopcornFX::PActionInstance_ & action);
	void							CheckForDead();
	bool							PerpareActionParameters();
...
private:
	...
	TPopcornFXRefPtrWrap<PopcornFX::CActionInstance>		m_ActionInstancePtr;
	TPopcornFXRefPtrWrap<PopcornFX::CActionParameters>		m_ActionParametersPtr;

	bool							m_Started : 1;
	bool							m_Stopped : 1;
	bool							m_DiedThisFrame : 1;
	bool							m_TeleportThisFrame : 1;
#if WITH_EDITOR
