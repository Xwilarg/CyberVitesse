...
static bool		_CheckFieldIsVisible(const PopcornFX::CBaseObject *hbo, const PopcornFX::HBO::CFieldDefinition &fieldDef)
{
	...
	if (fieldDef.VisibleInEditor(hbo) == PopcornFX::CBaseObject::FieldVisible)
		return true;
	//PopcornFX::CLog::Log(HH_INFO, "Ignored hidden field %s :: %s", hbo->RawName().Data(), fieldDef.m_Name);
	return false;
}
...
void	PopcornFX::GatherDependencies(const PopcornFX::CString &path, const PopcornFX::CbDependency &cb)
{
	...
	for (hh_u32 i = 0; i < objectList.Count(); i++)
	{
		const PopcornFX::CBaseObject	*hbo = objectList[i].Get();
		HH_ASSERT(hbo != null);
		if (hbo != null)
		{
			hh_u32	fieldCount = hbo->FieldCount();
			for (hh_u32 field = 0; field < fieldCount; field++)
			{
				...
				if (fieldType.m_Type == PopcornFX::HBO::GenericType_String)	// text string
				{
					if (PopcornFX::HBO::CaracsIsPath(fieldAttribs.m_Caracs))	// tagged as a path
					{
						...
						{
							const PopcornFX::TArray<CString>	&strArray = hbo->GetField<PopcornFX::TArray<CString> >(field);
							for (hh_u32 j = 0; j < strArray.Count(); j++)
							{
								if (!strArray[j].Empty())
				...
				else if (fieldType.IsInputLink())	// link
				{
					// don't fetch hidden fields
					if (!_CheckFieldIsVisible(hbo, fieldDef))
						continue;
					...
					{
						const PopcornFX::TArray<PopcornFX::HBO::CLink>	&linkArray = hbo->GetField<PopcornFX::TArray<PopcornFX::HBO::CLink> >(field);
						for (hh_u32 j = 0; j < linkArray.Count(); j++)
						{
							const CString		filePath = CFilePath::StripFilename(linkArray[j].Path());
							if (!filePath.Empty())
					...
					else
					{
						...
						const CString			filePath = CFilePath::StripFilename(link.Path());
						if (!filePath.Empty())
							cb(filePath, *hbo, field);
					}
				}
