...
#include <hh_particles/include/Storage/MainMemory/storage_ram_allocator.h>
...
// nothing here needs it (for now)
#define NEEDS_HBOSCOPE_PROFILER		0
...
namespace
{
	...
	public:
		...
		HH_FORCEINLINE static bool	Less(const _TypeIt &it0, const _TypeIt &it1) { return it0->Footprint() > it1->Footprint(); }
		HH_FORCEINLINE static bool	LessOrEqual(const _TypeIt &it0, const _TypeIt &it1) { return it0->Footprint() >= it1->Footprint(); }
		HH_FORCEINLINE static bool	Equal(const _TypeIt &it0, const _TypeIt &it1) { return it0->Footprint() == it1->Footprint(); }
	};
#endif
}
...
#if (PKUE_HUD_MEM_NODES != 0)
	m_MemNodesCache = HH_NEW(PopcornFX::TArray<PopcornFX::CMemStatNode>);
#endif
}

APopcornFXHUDMemory::~APopcornFXHUDMemory()
{
#if NEEDS_HBOSCOPE_PROFILER
...
#if (PKUE_HUD_MEM_NODES != 0)
	HH_SAFE_DELETE(m_MemNodesCache);
#endif
}
...
{
	HH_NAMEDSCOPEDPROFILE_C("APopcornFXHUDMemory::Update", POPCORNFX_UE_PROFILER_COLOR);
	...
	HH_ASSERT(pageStats.m_Used >= pageStats.m_Unused);
	HH_ASSERT(pageStats.m_Allocated >= pageStats.m_Freed);
	HH_ASSERT(pageStats.m_AllocCount >= pageStats.m_FreeCount);
	...
	PopcornFX::CMemStats::CaptureProfileNodes(*m_MemNodesCache);
...
{
	HH_NAMEDSCOPEDPROFILE_C("APopcornFXHUDMemory", POPCORNFX_UE_PROFILER_COLOR);
	...
	const hh_u32		frameCount = 16;
	if (m_Frames.Count() != frameCount)
	{
		hh_u32		oldCount = m_Frames.Count();
		if (!m_Frames.Resize(frameCount))
			return;
		for (hh_u32 i = oldCount; i < m_Frames.Count(); ++i)
		{
			m_Frames[i].Clear();
		}
	}
	...
	m_MaxFrame = PopcornFX::HHMax(m_MaxFrame, m_CurrentFrame + 1);

	m_MergedFrame.Clear();

	for (hh_u32 framei = 0; framei < m_MaxFrame; ++framei)
	{
...
{
	HH_NAMEDSCOPEDPROFILE_C("APopcornFXHUDMemory::Draw", POPCORNFX_UE_PROFILER_COLOR);
...
#define DRAW_POOL(__name, __pool)													\
	...
	if (y <= maxSlotListY)
	{
		...
		const hh_u32			memNodeCount = memNodes.Count();
		for (hh_u32 i = 0; i < memNodeCount; ++i)
		{
			if (y > maxSlotListY)
				break;
