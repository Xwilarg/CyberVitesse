...
{
	HH_ASSERT(PopcornFXEmitterComponent != null);
	HH_ASSERT(PopcornFXEmitterComponent->SceneValid());
...
bool	APopcornFXEmitter::EditorSpawnSceneIFN()
{
	...
	if (!EmitterComponent->ResolveScene(true))
	{
		...
		if (HH_VERIFY(scene != null && scene->PopcornFXSceneComponent != null))
		{
			scene->PopcornFXSceneComponent->SceneName = EmitterComponent->SceneName;
			HH_ONLY_IF_ASSERTS(bool sceneResolved = )EmitterComponent->ResolveScene(true);
			HH_ASSERT(sceneResolved);
			HH_ASSERT(EmitterComponent->Scene == scene);
		}
	}
