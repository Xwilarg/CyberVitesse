...
:	TCommands<FPopcornFXEffectEditorCommands>("PopcornFXEffectEditor", LOCTEXT("PopcornFXEffectEditor", "Effect Editor"), NAME_None, FEditorStyle::GetStyleSetName())
{
...
{
	// Viewport "Show" menu
	UI_COMMAND(TogglePreviewFloor, "Floor", "Toggles the preview floor visibility", EUserInterfaceActionType::ToggleButton, FInputChord());
	UI_COMMAND(TogglePreviewCubemap, "Cubemap", "Toggles the preview cubemap visibility", EUserInterfaceActionType::ToggleButton, FInputChord());
	...
	UI_COMMAND(LoopEmitter, "Loop emitter", "Toggles lopping of this emitter", EUserInterfaceActionType::ToggleButton, FInputChord());
}
...
void		FPopcornFXEffectEditor::RegisterTabSpawners(const TSharedRef<class FTabManager>& InTabManager)
{
	...
	InTabManager->RegisterTabSpawner(PropertiesTabId, FOnSpawnTab::CreateSP(this, &FPopcornFXEffectEditor::SpawnTab_Properties))
		...
		.SetIcon(FSlateIcon(FEditorStyle::GetStyleSetName(), "ClassIcon.PopcornFXEffect"));
...
{
	HH_ASSERT(EffectDetailsView.IsValid());
	HH_ASSERT(EffectAttributesView.IsValid());
	HH_ASSERT(m_Effect == file);
...
void	FPopcornFXEffectEditor::InitEffectEditor(const EToolkitMode::Type Mode, const TSharedPtr< class IToolkitHost >& InitToolkitHost, UPopcornFXEffect* Effect)
{
	...
	HH_ASSERT(!m_OnFileChangedHandle.IsValid());
...
void	FPopcornFXEffectEditor::BindCommands()
{
	...
	HH_ASSERT(PreviewViewport.IsValid());
	ToolkitCommands->MapAction(
