	...
	IPropertyTypeCustomizationUtils& CustomizationUtils)
{
	...
	TSharedRef<IPropertyHandle>		userData = PropertyHandle->GetChildHandle(GET_MEMBER_NAME_STRING_CHECKED(FPopcornFXSubRendererMaterial, UserData)).ToSharedRef();
	check(userData->IsValidHandle());
	...
	HH_STATIC_ASSERT(HH_ARRAY_COUNT(m_Thumbs) == 4);
	...
	HeaderRow.NameContent()
		...
		[
			...
			[
				...
				+ SHorizontalBox::Slot()
					.AutoWidth()
					[userData->CreatePropertyValueWidget()]
			]
			+ SVerticalBox::Slot()
				[
	...
	for (int32 i = 0; i < HH_ARRAY_COUNT(m_Thumbs); ++i)
	{
...
{
	check(thumbId >= 0 && thumbId < HH_ARRAY_COUNT(m_Thumbs));
...
void	FPopcornFXCustomizationSubRendererMaterial::OnThumbnailPtyChange(int32 thumbId)
{
	...
	check(thumbId < HH_ARRAY_COUNT(m_Thumbs));
	if (!m_Thumbs[thumbId].m_Pty->IsValidHandle())
		return;
	UObject		*obj;
	m_Thumbs[thumbId].m_Pty->GetValue(obj);
